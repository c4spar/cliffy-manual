...
`from` is optional. When it's omitted, the upgrade always runs instead of
comparing against the installed version. `force: true` does the same for a known
version.
...
For a script install, `args` is passed to the runtime's install command (e.g.
Deno permission flags) and `main` is the entrypoint module appended to the
registry url. `verbose: true` keeps the install command's own output, which is
silenced otherwise. The `runtime` option overrides `args` and `main` per
runtime, and adds `importMap` on Deno:
