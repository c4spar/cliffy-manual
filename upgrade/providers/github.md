...
The github provider lists all tags and branches. Disable branch listing with the
`branches` option (`new GithubProvider({ repository, branches: false })`). Lists
...
asset filename with the `asset` option:
...
map, `asset` also accepts a function that builds the filename per target:
...
For a private repository, a github token is resolved from the `token` option (a
string, or a function returning one), then from the `GITHUB_TOKEN` and
`GH_TOKEN` environment variables. The function form lets you plug in any scheme
(an OIDC exchange, `gh auth token`, a credential manager):
