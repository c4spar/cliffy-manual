...
The gitlab provider lists all tags and branches. Disable branch listing with the
`branches` option (`new GitlabProvider({ repository, branches: false })`). Lists
longer than `25` entries are rendered as a table.
...
release asset link with the `asset` option:
...
The `os` and `arch` keys use the Deno naming convention
(`darwin`/`linux`/`windows`, `x86_64`/`aarch64`) on every runtime. Instead of a
map, `asset` also accepts a function that builds the name per target:
...
For a private repository, a gitlab token is resolved from the `token` option (a
string, or a function returning one), then from the `GITLAB_TOKEN` environment
variable. It's sent as the `PRIVATE-TOKEN` header on api requests, and on the
asset download when the link points at the same host:

```ts ignore
new GitlabProvider({
  repository: "group/my-cli",
  asset: {/* … */},
  token: () => readTokenFromCredentialStore(),
});
```
